...
# Function is used to store the user token in a local file on the device
# The file is encrypted with a random password generated by the device so it canot be hacked
# The token can be used to keep a user signed in after they close the app and reopen
func store_user_token_local(user_token):
    var save_data = {
        sd_user_token = user_token
        }
...
# Function is used to pull the local stored user token from the encrypted file
# Stores the token into the stored_user_token variable to be used later
func load_user_token_local():
    var save_data_file = File.new()
    ...
    stored_user_token = current_line.sd_user_token
